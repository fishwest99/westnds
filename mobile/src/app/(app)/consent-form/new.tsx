...
type ConsentFormData = {
  ...
  ackInformedConsent: boolean;
  ackAssignmentRights: boolean;
  ackAuthRelease: boolean;
  ackSurpriseBalance: boolean;
  ackFinancialResp: boolean;
...
const defaultForm: ConsentFormData = {
  ...
  symptomOtherText: "",
  ackInformedConsent: false, ackAssignmentRights: false, ackAuthRelease: false,
  ackSurpriseBalance: false, ackFinancialResp: false,
...
function Checkbox({ label, value, onChange, error }: { label: string; value: boolean; onChange: (v: boolean) => void; error?: boolean }) {
  const hasError = error && !value;
  return (
    <TouchableOpacity style={styles.checkRow} onPress={() => onChange(!value)} activeOpacity={0.7}>
      <View style={[styles.checkBox, value && styles.checkBoxChecked, hasError && styles.checkBoxError]}>
        {value ? <Text style={styles.checkMark}>✓</Text> : null}
      </View>
      <Text style={[styles.checkLabel, hasError && styles.checkLabelError]}>{label}</Text>
    </TouchableOpacity>
  );
}

export default function NewConsentFormScreen() {
  ...
  const handleEmailPdf = async () => {
    if (Platform.OS === "web") {
      ...
      try {
        const result = await sharePdfOnWeb({
          url: `${process.env.EXPO_PUBLIC_BACKEND_URL}/api/consent-forms/${formId}/pdf`,
          filename: `consent-form-${formId}.pdf`,
          title: `West NDx Consent Form - ${form.patientName || "Patient"}`,
          text: `Please find attached the completed consent form.\n\nPatient: ${form.patientName || ""}\nDate of Service: ${form.dateOfService || ""}\nSurgeon: ${form.surgeonName || ""}`,
        });
        if (result === "failed") setInlineError("Failed to share PDF. Please try again.");
      } finally {
    ...
    try {
      ...
      await MailComposer.composeAsync({
        ...
        body: `Please find attached the completed consent form.\n\nPatient: ${form.patientName || ""}\nDate of Service: ${form.dateOfService || ""}\nSurgeon: ${form.surgeonName || ""}`,
        attachments: [fileUri],
      });
    } finally {
      setEmailLoading(false);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]} testID="new-consent-form-screen">
      <View style={styles.header}>
      ...
      <ScrollView style={styles.container} contentContainerStyle={[styles.content, isTablet && styles.contentTablet]}>
        <View style={[styles.inner, isTablet && styles.innerTablet]}>
          ...
          <View style={styles.section}>
            ...
            <Checkbox label="I have read and acknowledge the above" value={form.ackInformedConsent} onChange={(v) => update("ackInformedConsent", v)} error={showAckErrors} />
            ...
            <Checkbox label="I have read and acknowledge the above" value={form.ackAssignmentRights} onChange={(v) => update("ackAssignmentRights", v)} error={showAckErrors} />
            ...
            <Checkbox label="I have read and acknowledge the above" value={form.ackAuthRelease} onChange={(v) => update("ackAuthRelease", v)} error={showAckErrors} />
            ...
            <Checkbox label="I have read and acknowledge the above" value={form.ackSurpriseBalance} onChange={(v) => update("ackSurpriseBalance", v)} error={showAckErrors} />
            ...
            <Checkbox label="I have read and acknowledge the above" value={form.ackFinancialResp} onChange={(v) => update("ackFinancialResp", v)} error={showAckErrors} />
          ...
          {isSubmitted ? (
            <View>
              ...
              <TouchableOpacity style={styles.newFormButton} onPress={handleStartNew} testID="start-new-form-button">
                <Text style={styles.newFormButtonText}>Start New Form</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  checkBoxError: { borderColor: "#e53e3e", backgroundColor: "#fff5f5" },
