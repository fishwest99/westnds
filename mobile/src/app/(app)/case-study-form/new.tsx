...
const ELECTRODE_SITES = ["CPZ", "C3", "C4", "FPZ", "A1", "A2", "C1", "C2"];
...
export default function NewCaseStudyFormScreen() {
  ...
  const loadForm = async () => {
    try {
      const result = await api.post<CaseStudyFormData & { id: string }>("/api/case-study-forms", {});
      if (!result || !result.id) {
  ...
  const saveForm = useCallback(
    (updatedForm: CaseStudyFormData) => {
      ...
      saveTimeout.current = setTimeout(async () => {
        setSaving(true);
        try {
          await api.put(`/api/case-study-forms/${formId}`, {
            ...updatedForm,
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            setSubmitting(true);
            try {
              await api.post(`/api/case-study-forms/${formId}/submit`, {});
  ...
  const downloadPdf = async (): Promise<string | null> => {
    ...
    const destination = new FSFile(Paths.cache, `case-study-form-${formId}.pdf`);
    try {
      const result = await FSFile.downloadFileAsync(
        `${process.env.EXPO_PUBLIC_BACKEND_URL}/api/case-study-forms/${formId}/pdf`,
        destination,
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]} testID="new-case-study-form-screen">
      <View style={styles.header}>
        ...
        <View style={[styles.inner, isTablet && styles.innerTablet]}>
          ...
          {isSubmitted ? (
            <View>
              ...
              >
                <Text style={styles.newFormButtonText}>Start New Checklist</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <TouchableOpacity
