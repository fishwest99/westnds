...
export default function NewBillingFormScreen() {
  ...
  const loadForm = async () => {
    try {
      const result = await api.post<BillingFormData & { id: string }>("/api/billing-forms", {});
      if (!result || !result.id) {
  ...
  const saveForm = useCallback((updatedForm: BillingFormData) => {
    ...
    saveTimeout.current = setTimeout(async () => {
      setSaving(true);
      try {
        await api.put(`/api/billing-forms/${formId}`, updatedForm);
      } finally {
        setSaving(false);
      }
    }, 800);
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            setSubmitting(true);
            try {
              await api.post(`/api/billing-forms/${formId}/submit`, {});
  ...
  const downloadPdf = async (): Promise<string | null> => {
    ...
    const destination = new FSFile(Paths.cache, `billing-form-${formId}.pdf`);
    try {
      const result = await FSFile.downloadFileAsync(
        `${process.env.EXPO_PUBLIC_BACKEND_URL}/api/billing-forms/${formId}/pdf`,
        destination,
  ...
  const handleEmailPdf = async () => {
    ...
    try {
      ...
      await MailComposer.composeAsync({
        ...
        body: `Please find attached the completed billing sheet.\n\nPatient: ${form.patientName || ""}\nDate: ${form.date || ""}\nInvoice #: ${form.invoiceNumber || ""}`,
        attachments: [fileUri],
      });
    } finally {
    ...
    return (
      <SafeAreaView style={styles.safe} edges={["top"]}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>New Billing Sheet</Text>
          <View style={{ width: 60 }} />
        </View>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]} testID="new-billing-form-screen">
      <View style={styles.header}>
          ...
          {isSubmitted ? (
            <View>
              ...
              <TouchableOpacity
                ...
                testID="start-new-form-button"
              >
                <Text style={styles.newFormButtonText}>Start New Billing Sheet</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <TouchableOpacity
